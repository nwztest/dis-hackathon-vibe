...
import { devices, roomLabel } from "@/lib/mock-data";
    ...
    <AppShell>
      <main className="page-content">
        <div className="page-heading">
          <div>
            <h1>Devices</h1>
            <p>ESP32 sensor nodes assigned to privacy-preserving toilet and shower monitoring.</p>
          </div>
        ...
        <section className="table-panel">
          ...
          {devices.map((device) => (
            <article className="table-row device-table" key={device.id}>
              ...
              <span>{roomLabel(device.roomId)}</span>
              <span className={`status-pill ${device.status}`}>{device.status}</span>
              <span>{device.heartbeat}<small>{device.signal} · firmware {device.firmware}</small></span>
